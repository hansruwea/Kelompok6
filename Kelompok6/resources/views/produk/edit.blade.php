    ...
    <div class="col col-lg-6 col-md-6">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">Form Penjualan Elektronik</h3>
        ...
        <div class="card-body">
          ...
          <form action="{{ route('produk.update', $itemproduk->id) }}" method="post">
            ...
            <div class="form-group">
              <label for="kategori_id">Merk Elektronik</label>
              <select name="kategori_id" id="kategori_id" class="form-control">
                <option value="">Pilih Merk Elektronik</option>
                @foreach($itemkategori as $kategori)
                <option value="{{ $kategori->id }}" 
                {{ $itemproduk->kategori_id == $kategori->id ? 'selected' : ''}}>{{ $kategori->nama_kategori }}</option>
                @endforeach
              </select>
            </div>
            <div class="form-group">
              <label for="kode_produk">Kode Elektronik</label>
              <input type="text" name="kode_produk" id="kode_produk" value={{ $itemproduk->kode_produk }} class="form-control">
            </div>
            <div class="form-group">
              <label for="nama_produk">Nama Elektronik</label>
              <input type="text" name="nama_produk" id="nama_produk" value={{ $itemproduk->nama_produk }} class="form-control">
            </div>
            <div class="form-group">
              <label for="slug_produk">Diskon</label>
              <input type="text" name="slug_produk" id="slug_produk" value={{ $itemproduk->slug_produk }} class="form-control">
            </div>
            <div class="form-group">
              <label for="deskripsi_produk">Deskripsi Spesifikasi</label>
